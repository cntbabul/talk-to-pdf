...
function getVapi() {
    if (!vapi) {
        if (!VAPI_API_KEY) {
            throw new Error('NEXT_PUBLIC_VAPI_API_KEY is missing');
        }
...
export type CallStatus = 'idle' | 'connecting' | 'starting' | 'listening' | 'thinking' | 'speaking' | 'error';
...
export function useVapi(book: IBook) {
    ...
    // Use refs for values needed in callbacks to avoid stale closures and effect re-runs
    ...
    useEffect(() => {
            ...
            'speech-end': () => {
                ...
                if (message.transcriptType === 'final') {
                    ...
                    setMessages((prev) => {
                        ...
                        return isDupe ? prev : [...prev, { role: message.role, content: message.transcript, timestamp: new Date().toISOString() }];
                    });
                }
            },
        ...
        // Capture ref objects in local variables to satisfy the ESLint rule about ref values 
        // changing in cleanup functions. We specifically want the LATEST .current values 
        // at unmount time, so we capture the stable ref objects themselves.
        const currentSessionIdRef = sessionIdRef;
        const currentDurationRef = durationRef;

        return () => {
            // End active session on unmount using the latest values
            const sessionId = currentSessionIdRef.current;
            const durationValue = currentDurationRef.current;

            if (sessionId) {
                getVapi().stop();
                endVoiceSession(sessionId, durationValue).catch((err) =>
    ...
    }, [durationRef, maxDurationRef, sessionIdRef]);
